...
const EditHotel = ({match}) => {
  ...
  const {title, description, location, price, to, from, bed} = values;
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      await axios.put(`http://localhost:8001/api/hotel/update-hotel/${path}`,formdata, {
        headers: {
  ...
  const editHotels = async(req, res) => {
    ...
    await axios.get(`http://localhost:8001/api/hotel/${path}`).then((response) => {
      console.log("hjb",response);
      setValues({...values, ...response.data});
      setPreview(`http://localhost:8001/api/hotel/image/${response.data._id}`)
    })
                ...
                <div className='col-10'>
                  <form onSubmit={handleSubmit}>
                    <div>
                        ...
                        <Select value={bed} onChange={(value) => setValues({...values, bed: value})} className="w-100 m-2" size='large' placeholder="Number of beds">
                        ...
                        <button className='btn btn-outline-warning m-2'>Save</button>
                    </div>
